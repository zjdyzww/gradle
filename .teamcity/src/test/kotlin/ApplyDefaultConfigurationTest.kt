...
class ApplyDefaultConfigurationTest {
    ...
    fun expectedRunnerParam(daemon: String = "--daemon", extraParameters: String = "", os: Os = Os.LINUX): String {
        ...
        return "-Dorg.gradle.workers.max=%env.GBT_MAX_WORKERS% -PmaxParallelForks=%env.GBT_MAX_WORKERS% -s $daemon --continue $extraParameters \"-Dscan.tag.Check\" \"-Dscan.tag.\" -PteamCityBuildId=%teamcity.build.id% \"$expectedInstallationPaths\" -Porg.gradle.java.installations.auto-download=false"
    }
}
